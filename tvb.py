...
async def main():
    async with aiohttp.ClientSession() as client:
        async with client.get('https://www.tvbanywherena.com/cantonese/series/1244-AGeneralAScholarAndAnEunuch') as program:
            for episode in itertools.islice(bs4.BeautifulSoup(await program.text(), 'lxml').find('div', attrs={'class':'episodeDiv'}).find_all('a'), 22, None):
