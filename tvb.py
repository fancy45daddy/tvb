...
async def main():
    async with aiohttp.ClientSession(headers={'user-agent':fake_useragent.UserAgent().chrome}) as client:
        async with client.get('https://www.tvbanywherena.com/cantonese/series/494-ForensicHeroesIII') as program:
            for episode in itertools.islice(bs4.BeautifulSoup(await program.text(), 'lxml').find('div', attrs={'class':'episodeDiv'}).find_all('a'), 18, None):
