...
async def main():
    async with aiohttp.ClientSession(headers={'user-agent':fake_useragent.UserAgent().chrome}) as client:
        async with client.get('https://www.tvbanywherena.com/cantonese/series/2594-ForensicHeroesV') as program:
            for episode in itertools.islice(bs4.BeautifulSoup(await program.text(), 'lxml').find('div', attrs={'class':'episodeDiv'}).find_all('a'), 4, 5):
